...
import EShop.lab4.PersistentCartActorTest.cartActorWithCartSizeResponseOnStateChange
...
class PersistentCheckoutTest
  ...
  it should "be in cancelled state after cancel message received in selectingDelivery State" in {
    ...
    val checkoutActorAfterRestart: ActorRef = cartActorWithCartSizeResponseOnStateChange(system, id)
